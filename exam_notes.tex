...
\begin{quote}
    ...
    $$
    and $T = V - S$. The partition $(S, T)$ is then a cut. Now, consider a pair of vertices $u \in S$ and $v \in T$. \textcolor{red}{If $(u, v) \in E$, we must have $f(u, v) = c(u, v)$. If $(v, u) \in E$, we must have $f(v, u) = 0$}. If neither $(u, v)$ nor $(v, u)$ is in $E$, then $f(u, v) = f(v, u) = 0$. We thus have
    \begin{align*}
...
\begin{center}
    ...
    \end{tabular}
\end{center}
We see, that $x_1$ and $x_2$ are the only variables that can increase the value of the objective function. We choose $x_2$ as the pivot element (\textbf{DETTE ER VIGTIGT FOR AT FÅ ALGORTIMEN TIL AT TERMINERE EFTER 1 ITERATION - ALT ANDET VIL FÅ DEN TIL AT KØRE LÆNGERE. DET ER OGSÅ VIGTIGT, AT DEN UDSKIFTES HVOR $6$ ER POSITIV OG IKKE I DEN MODSATTE.}). Next, we isolate $x_2$ in $x_4$, since $x_4$ is the only constraint with a negative coefficient for $x_2$. The value of $x_2$ is then inserted in the other constraints as well as the objective function, resulting in
\begin{center}
    \begin{tabular}{l}
        $z = 12 - x_1 - 2x_4$ \\
        $x_2 = 6 - x_1 + x_3 - x_4$ \\
...
\end{figure*}
Thus, the edges $4$ and $4'$ is a min-cut, which is correct, however, the algorithm might not always return the correct output, as it has a probability of $\frac{2}{n(n - 1)}$ of returning a specific min-cut, where $n$ is the amount of vertices in the graph.
\\
\\
...
\begin{center}
    \begin{align*}
        ...
        &k \in \mathbb{N}\\
        &G \textcolor{red}{\text{ has a traveling-salesman tour with cost at most } k.}\}
    \end{align*}
\end{center}
This problem is also NP-complete, which I will be proving now.
\\
\\
\textcolor{red}{We first show that TSP belongs to NP. Given an instance of the problem, we use as a certificate the sequence of $n$ vertices in the tour. The verification algorithm checks that this sequence contains each vertex exactly once, sums up the edge costs, and checks whether the sum is at most $k$, which all in all can be done in polynomial time.}
\\
\\
\textcolor{red}{To prove that TSP is NP-hard, we show that $HAM-CYCLE \leq_p TSP$, that is, we transform an instace of TSP into an instance of HAM-CYCLE, and since HAM-CYCLE is NP-complete, then TSP must also be NP-complete.}
\\
\\
...
\begin{enumerate}
    \item Introduction
    \item \textit{Exact exponential algorithms} vs \textit{Parameterized algorithms} (vs \textit{approximation algorithms})
    \item \textit{Travelling salesman problem}:
    \begin{enumerate}
        \item Introduction
        \item Naive approach + running time analysis
        \item Dynamic programming + running time analysis
    \end{enumerate}
...
\begin{center}
    ...
    \end{math}
\end{center}
What is the running time of this algorithm? The number of path lengths computed is something along the lines of
$$
\sum_{j = 2} ^{n - 1} \binom{n - 1}{j} \sum_{i = 1} ^j (j - 1) \leq n^2 \sum_{j = 1} ^n \binom{n}{j} = n^2 \cdot 2^n.
$$
The algorithm thus has a running time of $O(n^2 \cdot 2^n)$ if we assume additions take at most polynomial time in $n$, which is far better than the previous algorithm's runtime of $O(n!)$. 
\\
\\
...
$$
|V| = \sum_{v \in V} 1 = \frac{1}{2} \sum_{v \in V} 2 \leq \frac{1}{2} \sum_{v \in V} d(v) \leq k^2.
$$
...
\begin{enumerate}
    ...
    \begin{enumerate}
        \item Introduction
        \item Introduction to \texttt{APPROX-VERTEX-COVER} with example
        \item Proof that \texttt{APPROX-VERTEX-COVER} is a 2-approximation algorithm
    \end{enumerate}
    \item MAX-3-CNF
    \begin{enumerate}
        \item Introduction
        \item Introduction to the randomized algorithm for \texttt{MAX-3-CNF}
...
\\
\\
Finding such a graph is NP-complete, however, we can use an approximation algortihm to find a vertex cover whose size is no more than twice the size of the optimal vertex cover, with a running time of $O(V + E)$, using adjacency lists to represent $E$. The algorithm works by continuously picking an arbitrary edge $(u, v)$, saving $u$ and $v$ in a set $C$, and removing $(u, v)$ and all other edges incident on either $u$ or $v$ from $G$. This is done untill $G$ has no edges left, where $C$ is returned *Do this while performing the algorithm on the following example*.
\begin{figure}[h]
    \centering
    \includegraphics[width = 6 cm]{entities/vertex_cover_approx.png}
\end{figure}
\\
\\
...
\\
\\
We start off by letting $A$ denote the set of edges that are picked in the approximation algorithm, $C^*$ be the optimal cover, and $C$ be the cover returned by the approximation algorithm. 
\\
\\
We know that $C$ is a vertex cover, since the algorithm loops until every edge in $E$ has been covered by some vertex in $C$.
\\
\\
We have
$$
|C^*| \geq |A|,
$$
since no two edges in $A$ share an endpoints and thus no two edges in $A$ are covered by the same vertex from $C^*$. We also have
$$
|C| = 2|A|,
$$
\textcolor{red}{since each iteration picks an edge for which neither of its endpoints are already in $C$}. Thus, we have
$$
|C| = 2|A| \leq 2|C^*|.
$$
...
\begin{enumerate}
    ...
    \end{enumerate}
    \item $Y$-monotone pieces + triangulation
    \begin{enumerate}
        \item Motivation
        \item Introduction and handling \textit{Split}, \textit{merge}, \textit{start}, \text{end} and \text{regular vertices}
        \item Runtime analysis
    \end{enumerate}
...
\begin{figure*}[h]
    \centering
    \includegraphics[width = 6 cm]{entities/polygon_example.png}
\end{figure*}
...
\\
\\
In the $3$-coloring apprach we let $T_\mathcal{P}$ be a triangulation of the simple polygon $\mathcal{P}$. Then, we select a subset of vertices of $\mathcal{P}$, such that any triangle of $T_\mathcal{P}$ has at least one selected vertex, and then we place the cameras at the selected vertices.
\\
\\
To find the subset we assign each vertex of $\mathcal{P}$ a color: white, grey or black. In a 3-coloring of a triangulated polygon, every triangle has a white, black and gray vertex. Hence, if we place cameras at all gray vertics, say, we have guarded the whole polygon. By choosing the smallest color class to place the cameras, we guard $\mathcal{P}$ using at most $\left \lfloor \frac{n}{3} \right \rfloor$ cameras. We can compute the set of at most $\left \lfloor n/3 \right \rfloor$ camera positions in linear time.
\\
\\
But we need a fast algortihm for triangulating a simple polygon. To do this we decompose the polygon into so-called $y$-monotone pieces, and then triangulate the pieces. A simple polygon is \textit{y-monotone} if and only if any horizontal line intersects it in a connected set or not at all.
\\
\\
We can partition a polygon into monotone pieces by removing all split vertices and merge vertices. Lets cover the different types of vertices.
\begin{enumerate}
...
\\
\\
Now that I have introduced the various types of vertices, let's look at how we can remove split vertices and merge vertices.
\begin{itemize}
    \item Split vertex: We use a plane sweep method. The algorithm moves a line downward over the plane. For a split vertex $v_i$ let $e_j$ be the edge to the left of $v_i$ on the sweep line, and let $e_k$ be the edge to the right of $v_i$ on the sweep line. Then we can always connect $v_i$ to the lowest vertex in between $e_j$ and $e_k$, and above $v_i$. If there is no such vertex then we can connect $v_i$ to the upper endpoint of $e_j$ or to the upper endpoint of $e_k$. This vertex that $v_i$ is connect to is called the \textit{helper} of $e_j$ and is denoted by $helper(e_j)$.
    \item Merge vertex: Now we know how to get rid of split vertices. What about merge vertices. Suppose the sweep line reaches a merge vertex $v_i$. Let $e_j$ and $e_k$ be the edges immediately to the left and right of $v_i$ on the sweepline. Observe that $v_i$ becomes the new helper of $e_j$ when we reach it. We would like to connect $v_i$ to the highest vertex below the sweep line in between $e_j$ and $e_k$. We do not know the highest vertex below the sweep line when we reach $v_i$. But it is easy to find later on: when we reach a vertex $v_m$, that replaces $v_i$ as the helper of $e_j$, then this is the vertex we are looking for. It can happen that the helper of $e_j$ is not replaced anymore below $v_i$. In this case we can connect $v_i$ to the lower endpoint of $e_j$.
\end{itemize}
...
\end{figure*}
We thus have $4$ red nodes, $2$ yellow nodes and $2$ green nodes. Thus, optimal solution for this pieces would be to place cameras at each of the yellow nodes or at each of the green nodes.
...
\\
\\
The depth first search can be started from any node of $g(T_\mathcal{P})$; the three vertices of the corresponding triangle are colored white, gray and black. Now suppose, we reach a node $v$ in $g$, coming from a node $\mu$. Hence $t(v)$ and $t(\mu)$ share a diagonal. Since the vertices of $t(\mu)$ have already been colored, only one vertex remains to be colored. Because $g(T_\mathcal{P})$ is a tree, the other nodes adjacent to $v$ have not been visited yet, and we still have the freedom to give the vertex the remaining color. Thus, a triangulated simple polygon can always be 3-colored. However, is this optimal? Yes, it is! Why? Well, there are simple polygons that require $\left \lfloor n / 3 \right \rfloor$ cameras, for instance, a comb-shaped polygon.
\\
\\
