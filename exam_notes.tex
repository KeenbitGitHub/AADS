...
\\
\\
First off, what is an \textitbf{encoding}? Well, an encoding is a mapping to a binary string, given an object. I will be using the notation $\langle x \rangle$ to denote the encoding of an instance $x$ of a problem.
\\
\\
...
\\
\\
Let $G = (V, E)$ be an instance of $HAM-CYCLE$. We construct an instance of TSP as the following. We form the complete graph $G' = (V, E')$, where $E' = \{(i, j) : i, j \in V \text{ and } i \neq j\}$, and we define the cost function $c$ by
\begin{center}
    \begin{math}
        c(i, j) = 
        \begin{cases}
