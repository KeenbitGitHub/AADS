...
\begin{enumerate}
    ...
    \item Introduction to various terms:
    \begin{enumerate}
        \item \textit{Abstract problems}
        \item \textit{Encoding} 
        \item \textit{Accepting/Rejection} and \textit{Deciding}
...
\\
\\
An abstract problem is a binary relation on a set $I$ of a problem \textbfit{instance} (or input) and a set of problem \textitbf{solutions} - (each instance can have multiple solutions - think of a graph with multiple shortest paths). In this topic we are only concerned with \textbfit{deicsion problems}: those having a yes/no solution. Thus, we can view an abstract decision problem as a function that maps the instance set $I$ to the solution set $\{0, 1\}$.
\\
\\
An \textitbf{encoding} of a set of abstract objects is a mapping to the set of binary strings. Thus, an algorithm that solves some abstract decision problem actually takes an encoding of a problem instance as input. We say that an algorithm solves a concrete problem in time $O(T(n))$ if, when it is provided a problem instance $i$ of length $n = |i|$, the algorithm can produce the solution in $O(T(n))$ time. I will be using the notation $\langle x \rangle$ to denote the encoding of an instance $x$ of a problem.
\\
\\
We say that an algorithm $A$ \textbfit{accepts} a string $x \in \{0, 1\}^*$ if, given input $x$, the output of the algorithm $A(x) \in \{0, 1\}$ is $1$. The language \textbfit{accpeted} by an algorithm $A$ is thus the set of strings $L = \{x \in \{0, 1\}^* : A(x) = 1\}$.
\\
\\
However, there might strings that never terminates, thus $A$ neither accepts nor rejects them. For this reason we have the term \textbfit{decide}. We say that, if additional all strings not in $L$ are rejected by $A$, that is $A(x) = 0 \text{ for all } x \in \{0, 1\}^* \backslash L$, then $L$ is \textitbf{decided} by $A$. We further say, that $L$ is \textitbf{decided by } $A$ \textbfit{ in polynomial time if } $A$ \textbfit{ decides } $L$ \textitbf{ and runs in polynomial time on all strings}. 
\\
\\
...
To prove that TSP is NP-hard, we show that $HAM-CYCLE \leq_p TSP$. Let $G = (V, E)$ be an isntance of $HAM-CYCLE$. We construct an instance of TSP as follows. We form the complete graph $G' = (V, E')$, where $E' = \{(i, j) : i, j \in V \text{ and } i \neq j\}$, and we define the cost function $c$ by
\begin{center}
    \begin{math}
        c(i, j) = 
        \begin{cases}
...
We now show that graph $G$ has a hamiltonian cycle iff $G'$ has a tour of cost at most $+$. Suppose that graph $G$ has a hamiltonian cycle $h$. each edge in $h$ belongs to $E$ and thus has cost $0$ in $G'$. Thus, $h$ is a tour in $G'$ with cost $0$. Conversely, suppose that graph $G'$ has a tour $h'$ of cost at most $0$. Since the costs of the edges in $E'$ are $0$ and $1$, the cost of tour $h'$ is exactly $0$ and each edge on the tour must have cost $0$. Therefore, $h'$ constains only edges in $E$. We conclude that $h'$ is a hamiltonian cycle in graph $G$.
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item Bounded Search tree - example of running + running time analysis
    \end{enumerate}
